...
@@got_options?:
	; [MOD] Hitbox display: while Shift (focus) is held and the player is alive,
; draw a marker centered on the actual 8x8-pixel collision box.
; The marker is the empty miko16.bft cel right after the Full Power item
; (PAT_ITEM + IT_FULLPOWER + 1: patnum 51 in TH04, 43 in TH05), which the
; original game never references - free to be hand-painted as a hitbox icon.
; NOTE: master.lib GRCG boxfills never reach the visible page in TH04/TH05
; stage double-buffering; the direct z_super_roll_put path below is the one
; mechanism verified to render here (same one the option sprites use).
PLAYER_MARKER_W = 16
PLAYER_MARKER_H = 16
PLAYER_HITBOX_PATNUM = PAT_ITEM + 7
	cmp	_miss_time, 0
	jnz	@@no_hitbox_display
	cmp	_shiftkey, 0
	jz	@@no_hitbox_display
	call	@grcg_setmode_rmw$qv
	mov	ax, di
	add	ax, (PLAYER_W / 2) - (PLAYER_MARKER_W / 2)
	mov	dx, [bp+@@screen_y]
	add	dx, (PLAYER_H / 2) - (PLAYER_MARKER_H / 2)
	call	z_super_roll_put_tiny_16x16_raw pascal, PLAYER_HITBOX_PATNUM
