...
inline void start_exec() {
	...
	// [MOD] Esc in the shottype selection returns to the main menu.
	if(!shottype_menu()) {
		return;
	}
...
// [MOD] Pause-menu restart support (MAIN sets [unused_2] before returning to
// the OP): identical to start_game()/start_extra(), but without the shottype
// selection - the previous run's [shottype] and options are kept.
inline void start_exec_auto() {
	cfg_save();
	pi_load(0, "ts1.pi");
	text_clear();
	snd_kaja_func(KAJA_SONG_FADE, 15);
...
int main(void)
{
	...
	while(!quit) {
		input_reset_sense();
		// [MOD] Pause-menu "restart" (從頭開始): MAIN set [unused_2] before
		// handing control back to the OP. Jump straight into a new run with
		// the previous shottype/options instead of showing the main menu.
		if(resident->unused_2) {
			const uint8_t restart_kind = resident->unused_2;
			resident->unused_2 = 0;
			if(restart_kind == 1) {
				start_game_auto();
			} else {
