...
bool pascal shottype_menu(void)
{
	int input_locked = 0;
	...
	do {
		...
		if(!input_locked) {
			...
			// [MOD] Esc returns to the main menu; shottype_menu() then reports false.
	...
	palette_black_out(1);
	// [MOD] On a cancelled selection, return to the main menu exactly like
	// musicroom_menu() does in m_music.cpp: restore the menu background onto
	// both VRAM pages, and - unlike on the start path - keep the ts*.pi
	// character pictures loaded, so that re-entering this screen works.
	if(confirmed) {
		pi_free(0);
		pi_free(1);
		pi_free(2);
	} else {
		// The menu background is large, and the OP only has a limited amount
		// of memory for .PI buffers: free the character pictures first, or
		// the pi_load() below would fail and leave the menu background black.
		pi_free(0);
		pi_free(1);
		pi_free(2);
		graph_showpage(0);
		...
		pi_put_8(0, 0, 0);
		pi_free(0);
		// shottype_menu() ends with palette_black_out(1), which leaves
		// [PaletteTone] at 0; the menu background would then be shown in
		// black. Raise the tone back to 100 before showing the menu palette.
		PaletteTone = 100;
		palette_entry_rgb_show(MENU_MAIN_PALETTE_FN);
		graph_copy_page(0);
		graph_accesspage(0);
		// Reload the pictures for the next selection (like at OP startup).
		pi_load(2, "ts3.pi");
		pi_load(1, "ts2.pi");
	}
	// [MOD] [key_det] is shared with the main menu; the menu would otherwise
	// interpret the Esc that just closed this screen as "quit the game".
	key_det = 0;
	return confirmed;
}
