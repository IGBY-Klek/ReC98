...
// [MOD] FPS counter in the bottom-left corner of the screen, drawn via the
// text plane with the default PC-98 ROM font. Called once per game frame from
// th02_main.asm, and measured against master.lib's 60 Hz vsync_Count2
// hardware VSync counter. TRAM row 23 is the bottom edge of the playfield;
// row 24 is the bottom roll margin, whose TRAM cells corrupt tile rendering.
void far fps_counter_update(void)
{
	extern volatile unsigned int vsync_Count2;
	// [MOD] TH02's main.exe breaks at runtime whenever its DGROUP (_DATA +
	// _BSS) grows by even one byte, so this function must not add any state.
	// Instead, it reuses the 2 unused bytes of [_scroll_unused] from
	// th02_main.asm (only ever written once with 0, never read):
	// low byte  = 8-bit start of the current VSync window,
	// high byte = number of game frames counted within it.
	extern unsigned int scroll_unused;

	unsigned char now8 = static_cast<unsigned char>(vsync_Count2);
	unsigned char* const state = reinterpret_cast<unsigned char*>(&scroll_unused);
	unsigned char diff = static_cast<unsigned char>(now8 - state[0]);

	if(state[1] == 0 && (diff >= 60)) {
		state[0] = now8; // First call: prime the window without output.
		return;
	}
	state[1]++;
	if(diff < 60) {
	...
	unsigned long fps_x100 = (static_cast<unsigned long>(state[1]) * 6000UL);
	fps_x100 /= diff;

	char fps_text[19];
	// [MOD] Full-width (SJIS) glyphs only: writing half-width ASCII to row 23
	// via text_putsa() made the game's gaiji note glyphs on that row render
	// mirrored. ０ ０ ． ０ ０ 　 Ｆ Ｐ Ｓ
	fps_text[0] = 0x82; fps_text[1] = static_cast<char>(0x4F + ((fps_x100 / 1000UL) % 10));
	fps_text[2] = 0x82; fps_text[3] = static_cast<char>(0x4F + ((fps_x100 / 100UL) % 10));
	fps_text[4] = 0x81; fps_text[5] = 0x44; // ・ (mid-dot decimal point)
	fps_text[6] = 0x82; fps_text[7] = static_cast<char>(0x4F + ((fps_x100 / 10UL) % 10));
	fps_text[8] = 0x82; fps_text[9] = static_cast<char>(0x4F + (fps_x100 % 10));
	fps_text[10] = 0x81; fps_text[11] = 0x40; // 　
	fps_text[12] = 0x82; fps_text[13] = 0x65; // Ｆ
	fps_text[14] = 0x82; fps_text[15] = 0x6F; // Ｐ
	fps_text[16] = 0x82; fps_text[17] = 0x72; // Ｓ
	fps_text[18] = '\0';

	// Bottom-left corner of the playfield, like the TH04/TH05 counter.
	text_putsa(PLAYFIELD_TRAM_LEFT, 23, fps_text, TX_WHITE);
	state[0] = now8;
	state[1] = 0;
}
