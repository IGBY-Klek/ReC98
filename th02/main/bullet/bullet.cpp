...
void bullets_update_and_render(void)
{
	...
	for(i = 0; i < BULLET_COUNT; i++) {
		...
		if((bullet.size_type == BST_PELLET)) {
			// [MOD] Shrunk the player hitbox from +-5 / (y -8..+2) to a
			// centered +-3, matching the feel of TH04/TH05.
			if(
				(screen_left >= (-(PELLET_W / 2) + player_center_x() - 3)) &&
				(screen_left <  (-(PELLET_W / 2) + player_center_x() + 3)) &&
				(screen_top  >= (-(PELLET_H / 2) + player_center_y() - 3)) &&
				(screen_top  <= (-(PELLET_H / 2) + player_center_y() + 3))
			) {
			...
			// [MOD] Shrunk the player hitbox from +-11 / (y -12..+8) to a
			// centered +-3, matching the feel of TH04/TH05.
			if(
				(screen_left >= (-(BULLET16_W / 2) + player_center_x() - 3)) &&
				(screen_left <  (-(BULLET16_W / 2) + player_center_x() + 3)) &&
				(screen_top  >= (-(BULLET16_H / 2) + player_center_y() - 3)) &&
				(screen_top  <= (-(BULLET16_H / 2) + player_center_y() + 3))
			) {
				player_is_hit = PLAYER_HIT;
				goto remove;
