...
// [MOD] ZUN called vector2() via a near call, which causes a fixup overflow
// once the code grows past the 64 KiB reach of such calls. Use the regular
// (far) declaration from the header instead.
...
)
{
	...
	while(group_i < count) {
		...
		// [MOD] Replaced ZUN's byte-exact near-call hack with a plain call.
