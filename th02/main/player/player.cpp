...
void near player_update_and_render(void)
{
	...
	// [MOD] Holding Shift shows the hitbox point at the player center, drawn
	// with the smallest bullet sprite. (TH02 has no focus mode, so Shift is
	// free to use; the keyboard group byte is the same np21/BIOS area that
	// TH04 and TH05 read for Shift.)
	{
		uint8_t far *keygroup_14 = reinterpret_cast<uint8_t far *>(MK_FP(0, 0x538));
		if((*keygroup_14) & 0x01) {
			vram_y_t point_vram_y = scroll_screen_y_to_vram(
				point_vram_y, (player_topleft.y + (PLAYER_H / 2) - 8)
			);
			super_roll_put_tiny(
				(player_topleft.x + (PLAYER_W / 2) - 8),
				point_vram_y,
				PAT_BULLET16_BALL
			);
		}
