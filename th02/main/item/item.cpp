...
void pascal items_add_semirandom(screen_x_t left, screen_y_t top)
{
	...
	if(randring1_next16_ge_lt(0, 16) == 18) {
...
void near items_update_and_render(void)
{
	...
	// [MOD] TH10-style point collection: pull while the player is in the top
	// quarter of the playfield (or bombing). Local only - no new globals.
	const bool16 pull_to_player = (
		!player_is_hit &&
		((player_topleft.y <= (PLAYFIELD_TOP + (PLAYFIELD_H / 4))) || bombing)
	);
	...
	for(int i = 0; i < ITEM_COUNT; (i++, p++)) {
		...
		if(p->age != 0) {
			// [MOD] Pulled: home in on the player center. ([age] is unused in
			// ZUN code and doubles as the pull latch here.)
			const pixel_t PULL_SPEED = 7;
			const int target_left = (
				player_topleft.x + (PLAYER_W / 2) - (ITEM_W / 2)
			);
			const int target_top = (
				player_topleft.y + (PLAYER_H / 2) - (ITEM_H / 2)
			);
			const int d_left = (target_left - p_left);
			const int d_top  = (target_top  - p_top.screen);
			p_left += (d_left >  PULL_SPEED) ?  PULL_SPEED : (d_left < -PULL_SPEED) ? -PULL_SPEED : d_left;
			p_top.screen += (d_top > PULL_SPEED) ? PULL_SPEED : (d_top < -PULL_SPEED) ? -PULL_SPEED : d_top;
			*p_left_ptr = p_left;
			p_top_ptr->v = TO_SP(p_top.screen);
			p->velocity_y.v = 0;
			p->velocity_x_during_bounce = 0;
		} else if(pull_to_player) {
			// [MOD] Latch this item while the pull is active.
			p->age = 1;
