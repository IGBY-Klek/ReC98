...
loc_B263:
		...
		cmp	es:[bx+mikoconfig_t.demo_num], 0
		jnz	short loc_B287
		; [MOD] Manual quit from the pause menu: skip the game-over flow and
		; go straight back to the OP.
		cmp	word ptr es:[bx+mikoconfig_t.unused_1], 0
		jz	short @@MOD_no_manual_quit
		mov	word ptr es:[bx+mikoconfig_t.unused_1], 0
		jmp	short loc_B290
@@MOD_no_manual_quit:
		call	sub_C31F
		or	ax, ax
...
var_1		= byte ptr -1
		...
		mov	[bp+var_1], 0
		mov	byte_1DB74, 0
		; [MOD] Pause menu, translated to Traditional Chinese with a new third
		; option "û-ãÀ" (restart). Strings live in the code segment below
		; to keep the DGROUP size untouched.
		call	text_putsa pascal, (26 shl 16) + 12, cs, offset pausem_title, TX_WHITE + TX_UNDERLINE
		call	pausem_items_put

pausem_idle:
		call	@input_reset_sense$qv
		cmp	[bp+var_1], 0
		jnz	short pausem_menu_wait
		; no state yet: wait for the key to be released before arming input
		cmp	_key_det, 0
		jnz	pausem_frame
		mov	[bp+var_1], 1
		jmp	pausem_frame
...
pausem_menu_wait:
		cmp	[bp+var_1], 1
		jnz	pausem_confirm_state
		; menu is armed: handle one key press per release
		test	byte ptr _key_det, INPUT_UP
		jz	short pausem_no_up
		dec	byte_1DB74
		jns	short pausem_redraw
		mov	byte_1DB74, 2
		jmp	short pausem_redraw

pausem_no_up:
		test	byte ptr _key_det, INPUT_DOWN
		jz	short pausem_no_vert
		inc	byte_1DB74
		cmp	byte_1DB74, 3
		jb	short pausem_redraw
		mov	byte_1DB74, 0
pausem_redraw:
		call	pausem_items_put
		mov	[bp+var_1], 0
...
pausem_no_vert:
		...
		jz	short pausem_no_ok
pausem_ok:
		cmp	byte_1DB74, 0
		jnz	short pausem_sel_not_cont
		call	pausem_clear_all
		xor	ax, ax
		jmp	pausem_exit

pausem_sel_not_cont:
		cmp	byte_1DB74, 1
		jnz	short pausem_sel_quit
		; Restart: remember the request in [unused_2] (2 = Extra, 1 = normal),
		; then leave through the regular manual-quit flow, which ends up in OP.
		les	bx, _resident
		mov	al, _stage_id
		cmp	al, 5
		jne	short pausem_restart_normal
		mov	byte ptr es:[bx+mikoconfig_t.unused_2], 2
		jmp	short pausem_restart_done
pausem_restart_normal:
		mov	byte ptr es:[bx+mikoconfig_t.unused_2], 1
pausem_restart_done:
		call	pausem_clear_all
		mov	ax, 1
		jmp	pausem_exit
...
pausem_confirm_state:
		cmp	[bp+var_1], 3
		jnz	short pausem_confirm_choose
		cmp	_key_det, 0
		jnz	short pausem_frame
		mov	[bp+var_1], 4
		jmp	pausem_frame

pausem_confirm_choose:
		cmp	[bp+var_1], 4
		jnz	short pausem_frame
		test	byte ptr _key_det, INPUT_UP
		jnz	short pausem_confirm_flip
		test	byte ptr _key_det, INPUT_DOWN
		jz	short pausem_confirm_no_vert
pausem_confirm_flip:
		mov	al, 1
		sub	al, byte_1DB74
		mov	byte_1DB74, al
		...
		mov	[bp+var_1], 3
		jmp	pausem_frame
pausem_confirm_no_vert:
		test	byte ptr _key_det, INPUT_OK
		jnz	short pausem_confirm_ok
		test	byte ptr _key_det, INPUT_SHOT
		jz	short pausem_confirm_no_ok
pausem_confirm_ok:
		cmp	byte_1DB74, 0
		jnz	short pausem_confirm_yes
		; "No": back to the menu, on the quit item; re-arm requires release
		mov	byte_1DB74, 2
		call	pausem_items_put
		mov	[bp+var_1], 0
		jmp	pausem_frame
pausem_confirm_yes:
		; "Yes": plain quit, no restart request
		les	bx, _resident
		mov	byte ptr es:[bx+mikoconfig_t.unused_2], 0
		call	pausem_clear_all
		mov	ax, 1
		jmp	pausem_exit
pausem_confirm_no_ok:
		test	byte ptr _key_det, INPUT_CANCEL
		jz	short pausem_frame
		call	pausem_clear_all
		xor	ax, ax
		jmp	pausem_exit
...
pausem_exit:
		; Wait for the key to be released before returning, so that the main
		; loop's INPUT_CANCEL test cannot immediately reopen the menu.
		; [ax] carries the return value and must survive the wait loop.
		push	ax
pausem_exit_wait:
		call	@input_reset_sense$qv
		cmp	_key_det, 0
		jz	short pausem_exit_done
		call	@frame_delay$qi pascal, 1
		jmp	pausem_exit_wait
pausem_exit_done:
		pop	ax
		leave
		retn
...
; =============== [MOD] pause menu helpers ===============

pausem_items_put	proc near
		mov	al, byte_1DB74
		mov	ah, 0
		cmp	ax, 0
		jnz	short pausem_item1_sel
		call	text_putsa pascal, (23 shl 16) + 14, cs, offset pausem_str_cont, TX_WHITE + TX_UNDERLINE
		jmp	pausem_item1_done
pausem_item1_sel:
		call	text_putsa pascal, (23 shl 16) + 14, cs, offset pausem_str_cont, TX_MAGENTA
pausem_item1_done:
		mov	al, byte_1DB74
		mov	ah, 0
		cmp	ax, 1
		jnz	short pausem_item2_sel
		call	text_putsa pascal, (23 shl 16) + 15, cs, offset pausem_str_restart, TX_WHITE + TX_UNDERLINE
		jmp	pausem_item2_done
pausem_item2_sel:
		call	text_putsa pascal, (23 shl 16) + 15, cs, offset pausem_str_restart, TX_MAGENTA
pausem_item2_done:
		mov	al, byte_1DB74
		mov	ah, 0
		cmp	ax, 2
		jnz	short pausem_item3_sel
		call	text_putsa pascal, (23 shl 16) + 16, cs, offset pausem_str_quit, TX_WHITE + TX_UNDERLINE
		retn
pausem_item3_sel:
		call	text_putsa pascal, (23 shl 16) + 16, cs, offset pausem_str_quit, TX_MAGENTA
		retn
pausem_items_put	endp

pausem_confirm_put	proc near
		call	text_putsa pascal, (17 shl 16) + 14, cs, offset pausem_str_ask, TX_WHITE + TX_BLINK
		mov	al, byte_1DB74
		mov	ah, 0
		cmp	ax, 0
		jnz	short pausem_no_sel
		call	text_putsa pascal, (17 shl 16) + 15, cs, offset pausem_str_no, TX_WHITE + TX_UNDERLINE
		call	text_putsa pascal, (17 shl 16) + 16, cs, offset pausem_str_yes, TX_MAGENTA
		retn
pausem_no_sel:
		call	text_putsa pascal, (17 shl 16) + 15, cs, offset pausem_str_no, TX_MAGENTA
		call	text_putsa pascal, (17 shl 16) + 16, cs, offset pausem_str_yes, TX_WHITE + TX_UNDERLINE
		retn
pausem_confirm_put	endp

pausem_clear_all	proc near
		call	text_putsa pascal, (10 shl 16) + 12, cs, offset pausem_spaces, TX_WHITE
		call	text_putsa pascal, (10 shl 16) + 14, cs, offset pausem_spaces, TX_WHITE
		call	text_putsa pascal, (10 shl 16) + 15, cs, offset pausem_spaces, TX_WHITE
		call	text_putsa pascal, (10 shl 16) + 16, cs, offset pausem_spaces, TX_WHITE
		retn
pausem_clear_all	endp

pausem_title	db	8Eh, 62h, 92h, 0E2h, 0			; ´\
pausem_str_cont	db	20h, 0E3h, 8Bh, 0E3h, 94h, 97h, 56h, 9Dh, 45h, 20h, 0	; " |åJ2 "
pausem_str_restart	db	20h, 9Ch, 6Eh, 93h, 0AAh, 8Ah, 4Ah, 8Eh, 6Eh, 20h, 0	; " û-ãÀ "
pausem_str_quit	db	20h, 8Ch, 8Bh, 91h, 0A9h, 97h, 56h, 9Dh, 45h, 20h, 0	; " P_J2 "
pausem_str_ask	db	81h, 40h, 90h, 5Eh, 93h, 49h, 81h, 60h, 97h, 76h, 8Ch, 8Bh, 91h, 0A9h, 97h, 56h, 9Dh, 45h, 0EAh, 9Dh, 81h, 48h, 0	; Ñ^ÅP_J2ú?
pausem_str_no	db	81h, 40h, 0E9h, 78h, 0EAh, 9Bh, 93h, 49h, 0EAh, 97h, 81h, 43h, 95h, 73h, 91h, 0DEh, 95h, 73h, 91h, 0DEh, 81h, 42h, 0	; vÑl,  
pausem_str_yes	db	81h, 40h, 90h, 0A5h, 90h, 5Eh, 93h, 49h, 0EAh, 82h, 81h, 43h, 91h, 0DEh, 0EAh, 99h, 91h, 0DEh, 0EAh, 99h, 81h, 42h, 0	; /Ñ˜, w w
pausem_spaces	db	32 dup(20h), 0
...
loc_BDE8:
		...
		or	ax, ax
		jz	short loc_BE0F
		; [MOD] A manual quit is not a game over: remember it, so that
		; _main can skip the scoreboard and continue flow.
		les	bx, _resident
		mov	word ptr es:[bx+mikoconfig_t.unused_1], 1
		mov	_quit, 1
...
HUD_TEXT	segment	byte public 'CODE' use16
	...
	extern @fps_counter_update$qv:far ; [MOD] bottom-left FPS counter
...
byte_1DB74	db 0
		db 0
off_1DB76	dd aVV2
					; "ÇŸÇÒÇ∆Ç…èIóπÇµÇøÇ·Ç§ÇÃ"
off_1DB7A	dd aVdvVVBbvVVVV
					; "Ç§ÇªÇ≈Ç∑ÅBÇ∑Ç›Ç‹ÇπÇÒÅB"
...
aVV2		db 'ÇŸÇÒÇ∆Ç…èIóπÇµÇøÇ·Ç§ÇÃ',0
aVdvVVBbvVVVV	db 'Ç§ÇªÇ≈Ç∑ÅBÇ∑Ç›Ç‹ÇπÇÒÅB',0
aB@b@vVvbavtvVV	db 'Å@Å@ÇÕÇ¢ÅAÇ‚ÇﬂÇ‹Ç∑ÅBÅ@',0
aEMPTY	db '                                                ',0
...
; centered.
_BONUS_RANK       	db ' ìÔà’ìx',0

_BONUS_PLAYPERF   	db 'ÉXÉeÅ[ÉW',0
_BONUS_BOMBS      	db 'É{ÉÄ',0
_BONUS_LIVES      	db 'É~ÉX',0
_BONUS_START_BOMBS	db 'ËÀåÇèâä˙êî',0
_BONUS_START_LIVES	db 'ËÀñ≤èâä˙êî',0
_BONUS_POINT 	db 'ì_',0
_BONUS_TIMES 	db 'Å~',0
_BONUS_EQUALS	db 'ÅÅ',0
_BONUS_EXTRA_CLEAR       	db 'ÉNÉäÉA',0
_BONUS_EXTRA_LIVES       	db 'É~ÉXâÒêî',0
_BONUS_EXTRA_BOMBS       	db 'É{ÉÄâÒêî',0
_BONUS_EXTRA_SIGMA_FRAMES	db 'ÉNÉäÉAÉ^ÉCÉÄ',0
