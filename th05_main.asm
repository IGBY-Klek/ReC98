...
SLOWDOWN_TEXT segment word public 'CODE' use16
	...
	@FPS_COUNTER_UPDATE$QV procdesc near	; [MOD] bottom-right FPS counter (overlay.cpp)
SLOWDOWN_TEXT ends
...
_envp		= dword	ptr  0Ch
		...
		or	ax, ax
		jz	loc_AEA4	; [MOD] near: the restart block below grew _main
...
loc_AE89:
		...
		call	sub_AEA6
	cmp	_quit, Q_RESTART
	jnz	short @@MOD_not_restart
	; [MOD] Pause restart (saisho): leave the run and hand over to the OP.
	; With demo_num set to this marker, the OP starts a new game right away
	; (fresh process, stage 0). Re-entering stage 0 in place would corrupt
	; the shared patterns, and relaunching MAIN itself breaks the video.
	les	bx, _resident
	; An Extra run restarts the Extra stage, the main game restarts at
	; stage 0; the OP picks the new run based on this marker.
	mov	al, 0FFh
	cmp	byte ptr es:[bx+resident_t.stage], STAGE_EXTRA
	jnz	short @@MOD_restart_marker
	mov	al, 0FEh
@@MOD_restart_marker:
	mov	byte ptr es:[bx+resident_t.demo_num], al
	push	ds
	push	offset arg0	; "op"
	nopcall	@GameExecl$qnxc
	mov	_quit, Q_KEEP_RUNNING
	jmp	short @@MOD_loop_again
; ---------------------------------------------------------------------------
...
loc_AEBB:
		...
		jz	short loc_AED7
		cmp	ax, 1
		jnz	short @@MOD_pause_quit
		mov	_quit, Q_RESTART	; [MOD] pause restart
		jmp	short loc_AED7
; ---------------------------------------------------------------------------
...
loc_B055:
		...
		; [MOD] bottom-right FPS counter: after the HUD update, registers protected
		push	ax
		push	bx
		push	cx
		push	dx
...
include th04/formats/scoredat_code_asm.asm
	...
	@gameover_scoreboard_put$qv procdesc near
	@gameover_regist$qv procdesc near
SCORE_TEXT ends
...
var_2		= word ptr -2
		...
		enter	2, 0
		; Remember the current measure of the stage BGM so that it can be
		; resumed from here after a continue, and fade it out.
		mov	ah, KAJA_GET_SONG_MEASURE
		int	PMD
		mov	[bgm_measure_at_gameover], ax
		kajacall	KAJA_SONG_FADE, 4
		mov	byte_2C99C, 20h	; ' '
...
loc_FA18:
		...
		call	@overlay_wipe$qv
		call	@gameover_scoreboard_put$qv
		; Play the name-registration track (TH10+ style)
		call	snd_load pascal, ds, offset aName, SND_LOAD_SONG
		kajacall	KAJA_SONG_PLAY
		; TH02-style: name entry on the scoreboard first...
		call	@gameover_regist$qv
		; ...then wipe the table and ask about continuing.
		call	@overlay_wipe$qv
		call	sub_FAA3
		mov	ah, 0
		mov	[bp+var_2], ax
...
loc_FA57:
		cmp	[bp+var_2], 0
		jnz	short loc_FA7D
		; Fade out the name track, then restore the stage BGM on continue
		kajacall	KAJA_SONG_FADE, 4
		les	bx, off_20A86
		mov	al, _stage_id
		add	al, 30h	; '0'
		...
		call	snd_load
		kajacall	KAJA_SONG_PLAY
		; ...fast-forwarding to the measure the player died on.
		mov	ah, 1Ch	; KAJA_SONG_FASTFORWARD
		mov	dx, [bgm_measure_at_gameover]
		int	PMD
		mov	PaletteTone, 100
		call	far ptr	palette_show
...
@@angle		= byte ptr -1
		...
		xor	di, di
		jmp	loc_172EC
	; [MOD] Point of Collection: recompute the item magnet every frame
	; (top quarter of the playfield, or the vanilla bomb pull) so the
	; top-line suction also works when the per-frame tile renderer is not
	; running, e.g. during boss fights.
	mov	ax, _player_pos.cur.y
	cmp	ax, ((PLAYFIELD_H / 4) shl 4)
	jbe	short @@MOD_PoC_on
	cmp	_bombing, 0
	jnz	short @@MOD_PoC_on
	mov	_items_pull_to_player, 0
	jmp	short @@MOD_PoC_done
@@MOD_PoC_on:
	mov	_items_pull_to_player, 1
@@MOD_PoC_done:
...
aBOSS_FINAL_TIMEOUT	db 'à´óÏÇaÇèÇìÇìëﬁé°é∏îsÅIÅIÅ@Å@Å@Å@Å@Å@Å@Å~Å@ÇOÅDÇO',0
aPENALTY_6	db 'ükã@êîí¶î±ÅiÇUêlÅjÅ~Å@ÇOÅDÇR',0
aPENALTY_5	db 'ükã@êîí¶î±ÅiÇTêlÅjÅ~Å@ÇOÅDÇT',0
aPENALTY_4	db 'ükã@êîí¶î±ÅiÇSêlÅjÅ~Å@ÇOÅDÇV',0
aPENALTY_CONT_1	db '„îËêí¶î±ÅiÇPâÒÅjÅ@Å~Å@ÇOÅDÇW',0
aPENALTY_CONT_2	db '„îËêí¶î±ÅiÇQâÒÅjÅ@Å~Å@ÇOÅDÇU',0
aPENALTY_CONT_3	db '„îËêí¶î±ÅiÇRâÒÅjÅ@Å~Å@ÇOÅDÇS',0
aBONUS_EASY	db 'ìÔà’ìxâ¡ê¨ÅiÇdÇÅÇìÇôÅjÅ@Å@Å@Å@Å~Å@ÇOÅDÇT',0
aBONUS_NORMAL	db 'ìÔà’ìxâ¡ê¨ÅiÇmÇèÇíÇçÇÅÇåÅjÅ@Å@Å~Å@ÇPÅDÇO',0
aBONUS_HARD	db 'ìÔà’ìxâ¡ê¨ÅiÇgÇÅÇíÇÑÅjÅ@Å@Å@Å@Å~Å@ÇPÅDÇQ',0
aBONUS_LUNATIC	db 'ìÔà’ìxâ¡ê¨ÅiÇkÇïÇéÇÅÇîÇâÇÉÅjÅ@Å~Å@ÇPÅDÇS',0
aBONUS_STAGE	db 'ÇrÇsÇ`ÇfÇdÅ@ÇaÇèÇéÇïÇì',0
aBONUS_DREAM	db 'ÇcÇqÇdÇ`ÇlÅ@ÇaÇèÇéÇïÇì',0
aGRAZEX50	db 'éCíeéüêîÅ@Å~Å@Å@ÇTÇO',0
aPOINT_ITEMS	db 'Å@Å~Å@ÇhÇîÇÖÇçÅ@ÇaÇèÇéÇïÇìÅ@',0
aBONUS_NOMISS	db 'Å@Å{Å@No Miss Bonus',0
aBONUS_NOBOMB	db 'Å@Å{Å@No Bomb Bonus',0
aBONUS_TOTAL	db 'Å@Å@Å@ÇsÇnÇsÇ`Çk',0
aALL_CLEAR	db 'Ç`ÇkÇkÅ@ÇbÇåÇÖÇÅÇíÅ@Å@',0
aPLAYER_REM	db 'ôîÈPêlêîÅ@Å~ÇPÇOÇOÇOÇO',0
aPOINT_TOTAL	db 'ÇsÇèÇîÇÅÇåÅ@ÇoÇèÇâÇéÇîÅ@ÇaÇèÇéÇïÇì',0
