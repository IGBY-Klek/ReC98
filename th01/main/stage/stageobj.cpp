...
void stageobjs_init_and_render(int stage_id)
{
	...
	// ZUN bug: The original used a 50-entry array here, overflowing once a stage
	// had more than 50 cards. Stage 22 in SCENE7.DAT has 76 cards, so this
	// caused an endless flip-in loop and softlocked progression.
	int frame_for[STAGEOBJS_COUNT] = { 0 };
	// [total_frame] follows on the stack in the original explanation below.
	int total_frame = 0;
	while(1) {
