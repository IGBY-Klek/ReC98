...
// [MOD] Like start_game(), but without the interactive character selection.
// Used when the "saisho" option in MAIN.EXE's pause menu hands over to us.
void near start_game_auto(void)
{
	resident->stage = 0;
	resident->credit_lives = resident->cfg_lives;
	resident->credit_bombs = resident->cfg_bombs;
...
// [MOD] Like start_extra(), but without the interactive character selection.
// Used when the "saisho" option in MAIN.EXE's pause menu hands over to us
// while an Extra run is in progress.
void near start_extra_auto(void)
{
	resident->stage = STAGE_EXTRA;
	resident->credit_lives = 3;
	resident->credit_bombs = 2;
