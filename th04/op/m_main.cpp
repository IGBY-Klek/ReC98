...
void main(void)
{
	...
	cfg_load();
	// [MOD] Pause "saisho": MAIN.EXE sets [demo_num] to 0xFF (restart the
	// main game) or 0xFE (restart an Extra run) before quitting to us;
	// start the respective new run immediately, with the current character.
	int restart_kind = 0;
	if(resident->demo_num == 0xFF) {
		restart_kind = 1;
	} else if(resident->demo_num == 0xFE) {
		restart_kind = 2;
	}
	resident->demo_num = 0;
