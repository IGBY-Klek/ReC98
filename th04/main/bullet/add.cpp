...
{
	// [MOD] Workaround for the TH04 Kurumi Divide Error crash (ReC98 branch
	// `th04_0_ring_as_single_bullet`): on Easy difficulty with minimum rank,
	// the rank-based tuning can reduce Kurumi's aimed rings to 0 bullets,
	// which would divide by zero in the ring spawn code. Turn such 0-way
	// rings into 1-rings (single bullets), as chosen by the community.
