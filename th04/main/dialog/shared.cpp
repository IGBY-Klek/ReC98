...
// [MOD] Fast-forward: while the Ctrl key is held, print dialog text
// instantly and auto-advance through the waits for key input.
inline bool dialog_fast_forward(void) {
	// Ctrl shows up either in the PC-98 key matrix (0:538, like the other
	// keys) or in the BIOS shift-status byte (0:53A, as returned by the
	// int 18h AH=02h that the games use for Shift); accept both.
	uint8_t far *keygroup_14 = reinterpret_cast<uint8_t far *>(MK_FP(0, 0x538));
	uint8_t far *shift_status = reinterpret_cast<uint8_t far *>(MK_FP(0, 0x53A));
	return (((*keygroup_14) | (*shift_status)) & 0x10) != 0;
}

#define dialog_delay(speedup_cycle) { \
	if(dialog_fast_forward()) { \
		/* [MOD] Ctrl: print the text instantly */ \
	} else if(key_det == INPUT_NONE) { \
		frame_delay(2); \
