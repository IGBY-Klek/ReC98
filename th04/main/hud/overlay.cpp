...
#endif
// [MOD] FPS counter in the bottom-right corner of the screen, drawn via
// the text plane with the default PC-98 ROM font. Called once per rendered
// stage frame from th04_main.asm / th05_main.asm. TH04 and TH05 share this
// file, and the counter is measured against master.lib\'s 60 Hz
// vsync_Count2 hardware VSync counter.
void pascal near fps_counter_update(void)
{
	extern volatile unsigned int vsync_Count2;

	static unsigned int vsync_window_start;
	static unsigned int vsync_prev;
	static unsigned int frames_in_window;
	static bool16 initialized;
	static char fps_text[9] = "00.00 FPS";

	unsigned int vsync_now = vsync_Count2;
	if(!initialized) {
		vsync_window_start = vsync_now;
		vsync_prev = vsync_now;
		frames_in_window = 0;
		initialized = true;
		return;
	}
	if((vsync_now - vsync_prev) == 0) {
		return; // No new VSync since the last frame.
	}
	vsync_prev = vsync_now;
	frames_in_window++;

	if((vsync_now - vsync_window_start) >= 60) {
		unsigned long fps_x100 = (
			static_cast<unsigned long>(frames_in_window) * 6000UL
		);
		unsigned int vsyncs = (vsync_now - vsync_window_start);

		fps_x100 /= vsyncs;
		fps_text[0] = '0' + static_cast<char>((fps_x100 / 1000UL) % 10);
		fps_text[1] = '0' + static_cast<char>((fps_x100 / 100UL) % 10);
		fps_text[2] = '.';
		fps_text[3] = '0' + static_cast<char>((fps_x100 / 10UL) % 10);
		fps_text[4] = '0' + static_cast<char>(fps_x100 % 10);
		fps_text[5] = ' ';
		fps_text[6] = 'F';
		fps_text[7] = 'P';
		fps_text[8] = 'S';
	...
	// [MOD] Written every frame, like the game's own HUD text: whatever wipes
	// the TRAM area during the frame, our text is the last thing written and
	// therefore always survives into the displayed frame.
	// [MOD] Row 23 (BGM_TRAM_Y) is the game-owned bottom strip; row 24 (the

	// bottom roll margin) corrupts tile rendering when written to.

	text_putsa(PLAYFIELD_TRAM_LEFT, BGM_TRAM_Y, fps_text, TX_WHITE);
}
